...
with torch.no_grad():
    for batch_idx, sam in enumerate(train_loader):
        data, target, label = sam['image'].to(device=device,dtype=torch.float), sam['mask'].to(device=device,dtype=torch.float), sam['label'].to(device=device,dtype=torch.long)
